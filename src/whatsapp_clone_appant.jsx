...
export default function EnhancedChatApp() {
  ...
  return (
    <div className={`h-screen flex flex-col ${modoOscuro ? "bg-zinc-900 text-white" : "bg-white text-black"}`}>
      <div className="p-3 flex justify-between items-center border-b shadow">
      ...
      <div className="flex flex-1">
        <div className="w-1/4 border-r p-3 bg-gray-100 dark:bg-zinc-800">
          <h2 className="text-lg font-semibold mb-2">Contactos</h2>
          {contactos.map((c) => (
            <Card
              key={c.telefono}
              onClick={() => setChatActivo(c.telefono)}
              className={`mb-2 cursor-pointer transition-colors ${chatActivo === c.telefono ? "bg-blue-100 dark:bg-blue-800" : "hover:bg-gray-200 dark:hover:bg-zinc-700"}`}
            >
        ...
        <div className="flex-1 flex flex-col">
          <ScrollArea ref={chatRef} className="flex-1 p-4 space-y-2 overflow-y-auto">
            {mensajes.map((msg, i) => (
              <div
                key={i}
                className={`max-w-xs px-4 py-2 rounded-lg ${msg.tipo === "enviado" ? "bg-green-500 text-white ml-auto" : "bg-gray-300 dark:bg-zinc-700"}`}
              >
                {msg.es_audio ? (
                  <audio
                    controls
          ...
          <div className="p-4 border-t flex gap-2 items-center">
            ...
            {!grabando ? (
              <Button onClick={comenzarGrabacion} variant="secondary">🎙️</Button>
            ) : (
